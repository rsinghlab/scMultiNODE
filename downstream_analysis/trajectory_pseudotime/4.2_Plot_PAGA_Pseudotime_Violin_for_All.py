'''
Description:
    Visualize PAGA pseudotime estimation with violin plots.
...
def plotLineageTime_Monocle(data_name, res_df, lineage_list, lineage_name, model_name):
    ...
    for i, x in enumerate(lineage_time):
        ...
        ax_list.text(i, np.max(x) + 0.05, "med={:.2f}".format(median_val), ha='center', va='bottom', fontsize=12, color='red')
    ...
    plt.tight_layout()
    plt.savefig("./res/figs/PAGA_pseudotime_each/lineage_pseudotime-{}-{}.pdf".format(model_name, lineage_name[-1]), dpi=600)
    # plt.show()
    plt.close()
...
if __name__ == '__main__':
    ...
    for i in range(4):
        ...
        for m in [
            ...
            res_df = scanpy.read_h5ad("./res/data4PAGA/{}-{}-PAGA_pseudotime.h5ad".format(data_name, m)).obs
            res_df["pseudotime"] = res_df["dpt_pseudotime"]
