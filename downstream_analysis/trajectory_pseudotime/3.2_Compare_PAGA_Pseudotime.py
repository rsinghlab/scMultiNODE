...
def loadModelPseudoTime(data_name, model_list):
    save_filename = "./res/data4PAGA/{}-{}-PAGA_pseudotime.h5ad"
    model_res_dict = {}
    for m in model_list:
        m_anndata = scanpy.read_h5ad(save_filename.format(data_name, m))
        m_anndata.obs["pseudotime"] = m_anndata.obs["dpt_pseudotime"]
        model_res_dict[m] = m_anndata.obs
...
def comparePseudoTime(model_name_list, model_res_dict, lineage_list=[], lineage_id=""):
    save_filename = "./res/comparison/{}-PAGA-lineage-{}-comparison.npy".format(data_name, lineage_id)
    # -----
