# Description: Apply Monocle3 to AE latent variables.
...
expression_matrix <- read.csv(sprintf("./res/data4Monocle/coassay_cortex-Static_AE-%s-integrate.csv", modality), row.names=NULL, header=FALSE)
expression_matrix <- as(expression_matrix, "sparseMatrix")
expression_matrix <- t(expression_matrix)
print(dim(expression_matrix))

print("Loading umap latent...")
umap_matrix <- read.csv(sprintf("./res/data4Monocle/coassay_cortex-Static_AE-%s-umap_latent_data.csv", modality), row.names=NULL, header=FALSE)
umap_matrix <- as.matrix(umap_matrix)
print(dim(umap_matrix))

print("Loading cell meta data...")
cell_metadata <- read.csv(sprintf("./res/data4Monocle/coassay_cortex-Static_AE-%s-concat_meta_df.csv", modality), row.names=1)
...
write.csv(cell_metadata, sprintf("./res/data4Monocle/coassay_cortex-Static_AE-%s-Monocle3_res_df.csv", modality), row.names = FALSE)
