# Description: Apply Monocle3 to scMultiNODE integration.
...
expression_matrix <- read.csv("./res/data4Monocle/coassay_cortex-scMultiNODE-concat_integrate.csv", row.names=NULL, header=FALSE)
expression_matrix <- as(expression_matrix, "sparseMatrix")
expression_matrix <- t(expression_matrix)
print(dim(expression_matrix))

print("Loading umap latent...")
umap_matrix <- read.csv("./res/data4Monocle/coassay_cortex-scMultiNODE-umap_latent_data.csv", row.names=NULL, header=FALSE)
umap_matrix <- as.matrix(umap_matrix)
print(dim(umap_matrix))

print("Loading cell meta data...")
cell_metadata <- read.csv("./res/data4Monocle/coassay_cortex-scMultiNODE-concat_meta_df.csv", row.names=1)
...
write.csv(cell_metadata, "./res/data4Monocle/coassay_cortex-scMultiNODE-Monocle3_res_df.csv", row.names = FALSE)
