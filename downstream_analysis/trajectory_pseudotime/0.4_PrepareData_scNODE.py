'''
Description:
    Prepare scNODE single-modal latent variables for pseudotime estimation with Monocle3 or PAGA.
...
def loadscNODELatent(data_name, file_dir="./scNODE_latent"):
    rna_res = np.load("{}/{}-RNA-scNODE-res.npy".format(file_dir, data_name), allow_pickle=True).item()
    rna_data = rna_res["true"]
    rna_latent = rna_res["latent_seq"]
    rna_latent_concat = np.concatenate(rna_latent, axis=0)
    atac_res = np.load("{}/{}-ATAC-scNODE-res.npy".format(file_dir, data_name), allow_pickle=True).item()
    atac_data = atac_res["true"]
    atac_latent = atac_res["latent_seq"]
    atac_latent_concat = np.concatenate(atac_latent, axis=0)
    return rna_data, atac_data, rna_latent_concat, atac_latent_concat

# ======================================================

def _plotscNODELatent(latent_umap, all_tps, all_cell_types, modal_name):
...
def _perModalityAnalysis(modal_latent, modal_cell_types, modal_cell_tps, modal_name):
    ...
    save_filename = "./res/aux_data/{}-reduce-all-scNODE-{}_aux_data.npy".format(data_name, modal_name)
    ...
    # -----
    df_savefile = "./res/data4Monocle/{}-scNODE-{}-concat_meta_df.csv".format(data_name, modal_name)
    if not os.path.isfile(df_savefile):
        print("Saving aux data for Monocle...")
        obs_df = pd.DataFrame({
            "tps": modal_cell_tps,
        ...
        np.savetxt("./res/data4Monocle/{}-scNODE-{}-integrate.csv".format(data_name, modal_name), modal_latent,delimiter=",")
        np.savetxt("./res/data4Monocle/{}-scNODE-{}-umap_latent_data.csv".format(data_name, modal_name), umap_latent_data,delimiter=",")
    # -----
    # Plot scNODE latent
    _plotscNODELatent(umap_latent_data, modal_cell_tps, modal_cell_types, modal_name)


def scNODELatentAnalysis(rna_tps, atac_tps, rna_cell_types, atac_cell_types, latent_dim=50):
    print("-" * 70)
    print("Loading modality-specific scNODE latent...")
    (rna_data, atac_data, rna_latent, atac_latent) = loadscNODELatent(data_name, file_dir="./res/scNODE_latent")
...
if __name__ == '__main__':
    ...
    # -----
    # modality-specific scNODE latent
    scNODELatentAnalysis(rna_cell_tps, atac_cell_tps, rna_cell_types, atac_cell_types, latent_dim=50)
