# Description: Apply Monocle3 to baseline integration.
...
model_name <- "SCOTv1" # SCOTv1, SCOTv2, Pamona, UnionCom, uniPort, Seurat
print("Loading expression matrix...")
expression_matrix <- read.csv(sprintf("./res/data4Monocle/coassay_cortex-%s-concat_integrate.csv", model_name), row.names=NULL, header=FALSE)
expression_matrix <- as(expression_matrix, "sparseMatrix")
expression_matrix <- t(expression_matrix)
print(dim(expression_matrix))

print("Loading umap latent...")
umap_matrix <- read.csv(sprintf("./res/data4Monocle/coassay_cortex-%s-umap_latent_data.csv", model_name), row.names=NULL, header=FALSE)
umap_matrix <- as.matrix(umap_matrix)
print(dim(umap_matrix))

print("Loading cell meta data...")
cell_metadata <- read.csv(sprintf("./res/data4Monocle/coassay_cortex-%s-concat_meta_df.csv", model_name), row.names=1)
...
write.csv(cell_metadata, sprintf("./res/data4Monocle/coassay_cortex-%s-Monocle3_res_df.csv", model_name), row.names = FALSE)
