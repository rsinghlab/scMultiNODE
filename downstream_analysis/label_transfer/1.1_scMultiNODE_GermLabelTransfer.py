...
def loadLatent():
    res = np.load(
        "../../modal_integration/res/model_latent/{}-reduce-all-scMultiNODE-50dim.npy".format(data_name),
        allow_pickle=True
    ).item()
...
def germLabelTransfer(latent, germ_layer_list, test_latent):
    X = latent
    Y = germ_layer_list
    model = RandomForestClassifier()
    model.fit(X, Y)
    acc = accuracy_score(Y, model.predict(latent))
    print("Training Acc: {}".format(acc))
    Y_test = model.predict(test_latent)
    return Y_test, model
...
if __name__ == '__main__':
    ...
    # -----
    print("-" * 70)
    save_filename = "./res/germ_layer/{}-reduce-all-scMultiNODE-label_transfer.npy".format(data_name)
    ...
    if not os.path.isfile(save_filename):
        ...
        atac_germ_layer, class_model = germLabelTransfer(rna_integrate, rna_germ_layer, atac_integrate)
        # -----
        ...
        umap_model = data_res["umap_model"]
    # -----
